...
def train_drqn_tmaze(agent, environment, training_config, logger):
    ...
    for episode in range(training_config.num_episodes):
        ...
        # Logging
        if (episode + 1) % training_config.eval_period == 0:
            avg_reward = np.mean(episode_rewards[-training_config.eval_period:])
...
def train_gru_tmaze(agent, environment, training_config, logger):
    ...
    for episode in range(training_config.num_episodes):
        ...
        if (episode + 1) % training_config.eval_period == 0:
            avg_reward = np.mean(episode_rewards[-training_config.eval_period:])
            logger.info(f"Episode {episode + 1}/{training_config.num_episodes}, "
                       f"Avg Reward: {avg_reward:.3f}, Last Reward: {total_reward:.3f}")
    
    return episode_rewards


def train_drqn_mdp(agent, environment, training_config, logger):
...
def train_gru_mdp(agent, environment, training_config, logger):
    ...
    for episode in range(training_config.num_episodes):
        ...
        if trials:
            ...
            batch_rewards = np.array(conditions['training_guide'])  # (batch_size, 3)
            ...
            # Logging
            if (episode + 1) % training_config.eval_period == 0:
                avg_accuracy = np.mean(episode_rewards[-training_config.eval_period:])
                logger.info(f"Episode {episode + 1}/{training_config.num_episodes}, "
                           f"Loss: {loss:.4f}, Accuracy: {avg_accuracy:.3f}")
        else:
...
def main():
    ...
    # Setup logging
    logger = setup_logging(training_config.log_dir, experiment_name)
    ...
    if args.model == 'drqn':
        if args.environment == 'tmaze':
            episode_rewards = train_drqn_tmaze(agent, environment, training_config, logger)
        elif args.environment == 'mdp':
            episode_rewards = train_drqn_mdp(agent, environment, training_config, logger)
        else:
            raise ValueError(f"Unsupported environment {args.environment} for DRQN")
    elif args.model == 'gru':
        if args.environment == 'tmaze':
            episode_rewards = train_gru_tmaze(agent, environment, training_config, logger)
        elif args.environment == 'mdp':
            episode_rewards = train_gru_mdp(agent, environment, training_config, logger)
        else:
    ...
    model_path = os.path.join(training_config.save_dir, f"{experiment_name}.pt")
